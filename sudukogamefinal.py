...
def checkSolved():
    ...
    # Function to handle the key press events ('s' for solving, 'n' for new game, 'q' for quitting, and 'r' for resetting)
def key_pressed(event):
    ...
    if event.char.lower() == 's':
        ...
        grid = generateSudoku()
        draw_grid()  # Refresh grid for new puzzle
    elif event.char.lower() == 'q':
        root.quit()  # Quit the game when 'q' is pressed
    elif event.char.lower() == 'r':
        draw_grid()  # Reset the board with the same puzzle
    root.bind('<Key>', key_pressed)  # Bind key press events
def reset():
...
# Function to create buttons for Quit (q), Reset (r), Solve (s), and New Game (n)
def create_buttons():
    global start_time_label
    quit_button = tk.Button(root, text="QUIT", command=root.quit, bg='#b5c6f3', fg='black',font='anton')
    reset_button = tk.Button(root, text="RESET", command=reset,bg='#b5c6f3', fg='black',font='anton')
    solve_button = tk.Button(root, text="SOLVE", command=solve, bg='#b5c6f3', fg='black',font='anton')
    new_game_button = tk.Button(root,font='anton', text="NEW", command=new_game, bg='#b5c6f3', fg='black')
